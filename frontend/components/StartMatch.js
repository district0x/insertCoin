...
import { linkWalletToDiscord } from "../lib/database";
...
const StartMatch = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const matchAmountEth = (Number(matchAmount) / ethPriceInUsd).toFixed(18);
        ...
        onSuccess: async (result) => {
          try {
            ...
            if (receipt) {
              ...
              const matchId = processedReceipt.eventData.matchId;

              if (account && account.address) {
                await linkWalletToDiscord(account.address, matchId);
              }
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <Label htmlFor="matchAmount">Match Amount (USD)</Label>
              <Input
                type="number"
                name="matchAmount"
                id="matchAmount"
