...
import { recordMatchResult } from "../lib/database";
...
const CloseMatch = () => {
  ...
  const parseTransactionLogs = async (receipt) => {
    ...
    try {
      ...
      if (data.length < 5) {
        console.warn("Log data does not contain expected number of parameters");
        return null;
      }
        ...
        if (parsedData) {
          try {
            await recordMatchResult({
              ...
              loserAddress: receipt.from,
              winnerAmount: parsedData.winnerAmount,
              winnerAmountUsd: parsedData.winnerAmountUsd,
            ...
            setToast({
              show: true,
              message: `Match closed successfully! Transaction hash: ${receipt.transactionHash}`,
              type: "success",
            });
          } catch (error) {
