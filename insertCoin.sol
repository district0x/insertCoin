...
contract MVPCLR is OwnableUpgradeable {
    ...
    function createTournament(uint256 _tournamentId, uint256 _numEntrants, uint8 _winnersPercentage, uint8 _multisigPercentage) public onlyAdmin {
    // existing checks
    require(!tournaments[_tournamentId].isActive, "Tournament already exists");
    ...
    for (uint256 i = 0; i < winners.length; i++) {
        uint256 winnerPayout = totalDonations * winnersPercentages[i] / 100;
    ...
    uint256 allocatedForWinners = totalDonations * tournaments[_tournamentId].winnersPercentage / 100;
    require(totalPayout <= allocatedForWinners, "Payout exceeds allocated amount for winners");
    ...
    require(rounds[_roundId].isActive, "MVPCLR: closeRound - Round is not active");
    require(_winner == rounds[_roundId].player1 || _winner == rounds[_roundId].player2, "MVPCLR: closeRound - Invalid winner address");
    ...
    function startRound(address _player1Address, address _player2Address, uint256 _roundDuration, uint256 _betAmount) public payable {
    require(_player1Address != address(0) && _player2Address != address(0), "MVPCLR: startRound - Addresses cannot be zero");
    require(_player1Address != _player2Address, "MVPCLR: startRound - Player addresses must be different");
    require(!isBlacklisted[msg.sender], "MVPCLR: startRound - Sender is blacklisted");
    ...
    // receive donation for the matching pool, this function is incomplete
